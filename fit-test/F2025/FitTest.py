...
def plot_stacked(ax, column, max_score, show_xlabel=True, show_ylabel=True):
    ...
    if show_xlabel:
        ax.set_xlabel(
            "Score", fontsize=11, color=COLORS["text"], fontweight="500", labelpad=8
        )
    if show_ylabel:
        ax.set_ylabel(
            "Participants",
...
gs = fig.add_gridspec(
    2, 3, hspace=0.4, wspace=0.3, left=0.07, right=0.95, top=0.88, bottom=0.08
)
...
legend_elements = [
    mpatches.Patch(
        facecolor=COLORS["male"],
        edgecolor="white",
        linewidth=1.5,
        label=f"Male (n={n_males})",
    ),
    mpatches.Patch(
        facecolor=COLORS["female"],
        edgecolor="white",
        linewidth=1.5,
        label=f"Female (n={n_females})",
    ),
]
legend = fig.legend(
...
fig.text(
    0.5,
    0.955,
    "Fitness Assessment Score Distribution",
    ha="center",
    fontsize=18,
    weight="700",
    color=COLORS["text"],
...
fig.text(
    0.5,
    0.932,
    "Performance breakdown by gender across six components",
    ha="center",
    fontsize=11,
    style="italic",
    color=COLORS["text_light"],
...
plt.savefig(
    BASE_DIR / "images/fitness_tests.png",
    dpi=300,
    bbox_inches="tight",
...
ax.set_title(
    "Overall Fitness Score Distribution",
    fontsize=18,
    weight="700",
    pad=20,
    color=COLORS["text"],
...
ax.text(
    0.5,
    0.96,
    f"Combined assessment across all fitness components (max: {max_values['Total']})",
    transform=ax.transAxes,
    ha="center",
    fontsize=11,
    style="italic",
...
plt.tight_layout()
plt.savefig(
    BASE_DIR / "images/fitness_total.png",
    dpi=300,
    bbox_inches="tight",
    facecolor=COLORS["background"],
)
